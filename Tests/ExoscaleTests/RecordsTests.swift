...
func listRecordsResponseDecodesRecords() throws {
    ...
    let response = try Exoscale.jsonDecoder().decode(ListRecordsResponse.self, from: data)

    #expect(response.records.count == 1)
    #expect(response.records[0].updatedAt == Date(timeIntervalSince1970: 1777287600))
    ...
    #expect(response.records[0].createdAt == Date(timeIntervalSince1970: 1777284000))
